...
#### 2.1 **索引檔案（Index File）**
📁 **位置**：`assets/data/questions/questions.json`
- 由轉換腳本自動生成
- 包含應用程式配置和題目檔案清單
- 格式包含：
  ```json
  {
    "config": {
      ...
      "enableFavor": false,                 // 是否啟用清除我的最愛功能
      "configVersion": "government-procurement",  // 配置版本
      "displayName": "政府採購法"           // 題庫顯示名稱
    },
    "questionFiles": [                      // 題目檔案清單
      {
        "series_no": "2025122301",          // 期數編號
        "displayName": "工程及技術服務採購作業",  // 顯示名稱
        "file": "2025122301.json",          // 檔案路徑（相對於 questions/ 目錄）
        "count": 183,                       // 題目數量
        "testName": "GOVERNMENT_PROCUREMENT",  // 測驗名稱（可選）
        "subject": "L11"                    // 科目（可選，兩層結構時為 null）
      }
    ]
  }
  ```
...
**配置說明：**
- `config` 區塊包含應用程式的所有配置參數
- `questionFiles` 陣列包含所有題目檔案的元資料
- 支援兩層結構（無 subject）或三層結構（有 subject）
...
1. App 啟動
   ↓
2. 載入索引檔案 (assets/data/questions/questions.json)
   ├─ 讀取 config 區塊，載入應用程式配置
   ├─ 讀取 questionFiles 陣列，取得題目檔案清單
   └─ 儲存配置到 AppConfigService
   ↓
3. 檢查資料版本 (config.dataVersion)
   ↓
4. 如果版本不同：
...
5. 按需載入題目檔案
   ...
   ├─ 載入對應的 JSON 檔案（例如：2025122301.json）
   ├─ 從檔案路徑或 questionFiles 元資料補充 testName、subject、series_no 到題目物件
   └─ 使用快取機制避免重複載入
   ↓
...
2. **執行轉換**
   ...
   - 會自動：
     ...
     - 更新索引檔案：`assets/data/questions/questions.json`
...
### 題目資料
- ✅ 已備份：`assets/data/questions/questions.json`（在專案中）
- ✅ 題目檔案：`assets/data/questions/` 目錄下的 JSON 檔案
...
├── assets/
│   ├── data/
│   │   ├── questions.json                    # 索引檔案（應用程式配置和題目檔案清單）
│   │   └── questions/                       # 題目檔案目錄
...
1. **JSON 檔案位置**
   - `assets/data/questions/questions.json` 和所有題目檔案會被打包進 App
   - 修改後需要重新建置 App 才會生效
